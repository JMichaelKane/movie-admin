<script setup lang="ts">
	...
	const search = createDelayFunction(GetSourceMoviesByKeyword, 500); // 防抖函数
	...
	onMounted(() => {
		watchPostEffect(() => {
			if (keyword.value.trim() != "") {
				...
				search(route.params.id, keyword.value, num, page.value, data, pgCount);
			} else {
				GetSourceMovies(route.params.id as string, num, page.value, data, pgCount);
			}
		});
	});
	...
	function refresh() {
		if (keyword.value.trim() != "") {
			...
			GetSourceMovies(route.params.id as string, 20, page.value, data, pgCount);
		}
	}
</script>
